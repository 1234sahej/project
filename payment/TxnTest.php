...
<body>
    ...
    if (isset($_POST['name'])) {
	    ...
	    $orderid = "ORD" . $_POST['phone'] . $breed . rand(1000, 9999);
	    ...
	    if ($breed == 'chihuahua')
		    ...
		    $price = 6000;
	    if ($price == 0) {
		    $query = "insert into `details` (`name`,`email`,`phone`,`password`,`breed`,`payment`)values('$name','$email','$phone','$password','$breed','done')";
		    $result = mysqli_query($conn, $query);
		    header('Location: ../login.php');
	    } else {
			$query = "insert into `details` (`name`,`email`,`phone`,`password`,`breed`,`payment`)values('$name','$email','$phone','$password','$breed','not done')";
		    $result = mysqli_query($conn, $query);
	    }
    } else {
